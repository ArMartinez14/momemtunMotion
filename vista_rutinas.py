...
def ver_rutinas():
    ...
    # Resumen PNG
    st.markdown("<div class='hr-light'></div>", unsafe_allow_html=True)
    st.markdown("<h3 class='h-accent'>📸 Resumen de la sesión</h3>", unsafe_allow_html=True)

    semana_prev = (datetime.strptime(semana_sel, "%Y-%m-%d") - timedelta(days=7)).strftime("%Y-%m-%d")
    rutina_prev_doc = next((r for r in rutinas_cliente if r.get("fecha_lunes")==semana_prev), None)

    def _key_ex_basico(e: dict):
        return ((e.get("bloque") or e.get("seccion") or "").strip().lower(),
                (e.get("circuito") or "").strip().upper(),
                (e.get("ejercicio") or "").strip().lower())

    def _peso_logico(e: dict | None) -> float:
        if not e: return 0.0
        try:
            pa = e.get("peso_alcanzado", None)
            if pa not in (None,"",[]): return float(str(pa).replace(",", "."))
        except: pass
        try:
            pa, _, _ = _parsear_series(e.get("series_data", []))
            if pa is not None: return float(pa)
        except: pass
        try:
            p = float(str(e.get("peso","")).replace(",", "."))
            return p
        except: return 0.0

    def _reps_logicas(e: dict) -> int:
        for k in ["repeticiones","reps_max","reps_min","reps"]:
            v = e.get(k); s = str(v).strip()
            if s.isdigit(): return int(s)
            try: return int(float(s.replace(",", ".")))
            except: pass
        return 0

    def _ejercicios_dia_como_lista(rutina_doc: dict, dia_idx: str | int) -> list[dict]:
        dia_idx = str(dia_idx)
        data_dia = (rutina_doc.get("rutina", {}) or {}).get(dia_idx, [])
        return obtener_lista_ejercicios(data_dia)

    def _mapa_prev_por_key(rutina_prev_doc: dict | None, dia_idx: str | int) -> dict:
        if not rutina_prev_doc: return {}
        prev_list = _ejercicios_dia_como_lista(rutina_prev_doc, dia_idx)
        return {_key_ex_basico(e): e for e in prev_list if isinstance(e, dict)}

    def _ejercicios_workout_con_tendencia(rutina_doc, rutina_prev_doc, dia_idx):
        lista = _ejercicios_dia_como_lista(rutina_doc, dia_idx)
        prev_map = _mapa_prev_por_key(rutina_prev_doc, dia_idx)
        salida = []
        for e in lista:
            if (e.get("circuito","") or "").strip().upper() != "D": continue
            nombre = e.get("nombre") or e.get("ejercicio") or "Ejercicio"
            reps = _reps_logicas(e)
            peso_act = _peso_logico(e)
            prev = prev_map.get(_key_ex_basico(e))
            peso_prev = _peso_logico(prev) if prev else 0.0
            mejoro = (peso_act > peso_prev + 1e-9)
            salida.append({"nombre":nombre,"reps":reps,"peso":peso_act,"mejoro":mejoro,
                           "grupo":(e.get("grupo") or "General"),"series":int(e.get("series",0) or 0)})
        return salida

    def _focus_grupo_muscular(lista_ejs: list[dict]) -> tuple[str,int]:
        acc={}
        for e in lista_ejs:
            g = e.get("grupo") or "General"
            acc[g] = acc.get(g,0)+int(e.get("series",0) or 0)
        if not acc: return ("General",0)
        grupo, total = max(acc.items(), key=lambda kv: kv[1])
        return (grupo, total)

    ej_workout = _ejercicios_workout_con_tendencia(rutina_doc, rutina_prev_doc, dia_sel)
    focus_tuple = _focus_grupo_muscular(ej_workout)

    col_a, col_b = st.columns([1,1])
    with col_a:
        gym_name = st.text_input("Marca/Gimnasio", value="Motion Performance")
    with col_b:
        st.write("")
        generar = st.button("📸 Generar imagen resumen", use_container_width=True, type="secondary")

    if generar:
        fig = generar_tarjeta_resumen_sesion(
            nombre=nombre or "Atleta", dia_indice=dia_sel,
            ejercicios_workout=ej_workout, focus_tuple=focus_tuple, gym_name=gym_name
        )
        st.pyplot(fig, clear_figure=False)
        buf = BytesIO(); fig.savefig(buf, format="png", bbox_inches="tight"); buf.seek(0)
        file_name = f"resumen_{(nombre or 'atleta').replace(' ','_').lower()}_{semana_sel}_dia{dia_sel}.png"
        st.download_button("⬇️ Descargar PNG", data=buf, file_name=file_name, mime="image/png", use_container_width=True)
