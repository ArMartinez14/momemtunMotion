...
def crear_rutinas():
    ...
    for i, tab in enumerate(tabs):
        with tab:
            ...
            st.markdown("#### Warm Up y Work Out")
            for idx in range(len(st.session_state[dia_key])):
                fila = st.session_state[dia_key][idx]
                if fila.get("Sección") == "Work Out" and idx < 6:
                ...
                # Circuito + inputs
                fila_cols = st.columns([1, 3, 2, 2, 2, 2, 2, 2])
                fila["Circuito"] = fila_cols[0].selectbox("", ["A", "B", "C", "D", "E", "F"],
                ...
                busqueda = fila_cols[1].text_input("", value=fila["Ejercicio"], key=f"busqueda_{i}_{idx}", label_visibility="collapsed", placeholder="Ejercicio")
                coincidencias = [e for e in lista_ejercicios if busqueda.lower() in e.lower()] if busqueda else lista_ejercicios
                seleccion = st.selectbox("", coincidencias, key=f"selector_{i}_{idx}", label_visibility="collapsed")
                ...
                fila["Series"] = fila_cols[2].text_input("", value=fila["Series"], key=f"ser_{i}_{idx}", label_visibility="collapsed", placeholder="Series")
                fila["Repeticiones"] = fila_cols[3].text_input("", value=fila["Repeticiones"], key=f"rep_{i}_{idx}", label_visibility="collapsed", placeholder="Reps")
                fila["Peso"] = fila_cols[4].selectbox("", pesos, key=f"peso_{i}_{idx}", label_visibility="collapsed") if pesos else fila_cols[4].text_input("", value=fila["Peso"], key=f"peso_{i}_{idx}", label_visibility="collapsed", placeholder="Kg")
                fila["Tiempo"] = fila_cols[5].text_input("", value=fila["Tiempo"], key=f"tiempo_{i}_{idx}", label_visibility="collapsed", placeholder="Seg")
                fila["Velocidad"] = fila_cols[6].text_input("", value=fila["Velocidad"], key=f"vel_{i}_{idx}", label_visibility="collapsed", placeholder="Vel")
                fila["RIR"] = fila_cols[7].text_input("", value=fila["RIR"], key=f"rir_{i}_{idx}", label_visibility="collapsed", placeholder="RIR")

                # === Agregar progresiones ===
                if progresion_activa:
                    p = progresion_activa[-1]  # '1', '2' o '3'
                    prog_cols = st.columns([3, 2, 3, 3])
                    fila[f"Variable_{p}"] = prog_cols[0].selectbox("", ["", "peso", "velocidad", "tiempo", "rir", "series", "repeticiones"],
                    ...
                    fila[f"Operacion_{p}"] = prog_cols[2].selectbox("", ["", "multiplicacion", "division", "suma", "resta"],
            ...
            if st.button(f"➕ Agregar fila extra en {dias[i]}", key=f"add_extra_row_{i}"):
